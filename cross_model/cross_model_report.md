...
**Key finding:** Constraint engineering on GPT-4.1-mini (composite: 1.842) outperforms
GPT-5.2 baseline (composite: 1.700), demonstrating that structured safety guardrails
...
GPT-4.1-mini shows the largest improvement from constraints (+0.283 composite),
while GPT-5.2 shows the smallest (+0.050), suggesting that more capable models
...
cases when constrained. Llama constrained still has 16.7% under-triage (3 cases).
...
score (+0.142) and safety (+0.300), with a lower hard fail rate. This demonstrates that
...
- Cases failing constrained in ALL 3 models: [29, 30]

Case 26 (double insulin dose, meds bucket, HIGH risk) is the only universal baseline
...
1. **Constraints improve safety across all models**, with the largest gains in
   mid-tier models (GPT-4.1-mini: +0.667 safety, -83% hard fail reduction).
...
3. **HIGH-risk under-triage is nearly eliminated** by constraints in GPT-4.1-mini and
   GPT-5.2 (both achieve 0%), while Llama still leaks 16.7%.
