...
def main() -> None:
    ...
    baseline["model_escalation_label"] = baseline["model_escalation_code"].map(lambda x: ESC_MAP_BASELINE.get(x, ""))
    ...
    # Constrained: should already be canonical label
    constrained["model_escalation_code"] = constrained["model_escalation_raw"].map(lambda x: x.upper())
    constrained["model_escalation_label"] = constrained["model_escalation_code"].map(lambda x: ESC_MAP_BASELINE.get(x, x.lower()))
    ...
    falsy = {"0","false","no","n",""}
    df["hard_fail_bool"] = df["hard_fail_norm"].map(lambda x: True if x in truthy else (False if x in falsy else False))
    ...
    for c in contract_cols:
        df[c] = df[c].map(lambda x: _norm(x).lower()).replace({"y":"yes","n":"no","true":"yes","false":"no"})
